...
def test_pieces_and_stocks(db_session): 
...
def test_commande_flow(db_session):
...
def test_commandes_actions_annexes(db_session):
...
def test_admin_and_reset(db_session):
...
def test_rotation_stands(db_session): 
...
def test_database_utilities(db_session):
    ...
    data_db()
    db = SessionLocal()
    assert db.query(Piece).count() > 30
    db.close()
